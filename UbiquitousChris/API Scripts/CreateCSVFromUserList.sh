...
{
    for USER_NAME in $EMAIL_LIST; do
        echo "INFO: Checking $USER_NAME"
        curl -sku "$API_USER:$API_PASSWORD" -H "accept: text/xml" ${JPS_SERVER}JSSResource/users/name/$USER_NAME
        USER_XML="$(curl -sku "$API_USER:$API_PASSWORD" -H "accept: text/xml" ${JPS_SERVER}JSSResource/users/name/$USER_NAME | xmllint --xpath '/users/name/text()' -)"
